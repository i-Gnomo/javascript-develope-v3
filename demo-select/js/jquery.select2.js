    ...
    $.fn.extend({
        ...
        oListData: function(index) {
            ...
            function dcallback(result) {
                ...
                if (ots.abcTag) {
                    //abc
                    abc = $.arrayfilter($.map(data, function(a) { return (ots.type != 'javascript' ? (a["fletter"] || a[ftitle].substring(0, 1)) : a["L"]) || '' }));
                    if (abc.length > 0) {
                        ...
                        for (var i = 0; i < abc.length; i++) {
                            ...
                            $.each(data, function(_i, item) {
                                if ((ots.type != 'javascript' ? (item["fletter"] || item[ftitle].substring(0, 1)) : item["L"]) == abc[i]) {
                                    var fname = item[ftitle];
                                    if (ots.type != 'javascript' && !item["fletter"]) {
            ...
            switch (type) {
                ...
                case 'bindListClick':
                    ...
                    _t.off('click').on('click', '.select-dl dl dd a', function(e) {
                        ...
                        var _ncons = _zselect.find('.select-box-content[data-level]');
